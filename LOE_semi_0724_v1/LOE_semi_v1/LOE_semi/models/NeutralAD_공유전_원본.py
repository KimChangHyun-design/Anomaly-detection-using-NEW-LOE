        ...
        for data in train_loader:
            ...
            if epoch <=self.warmup:
                ...
                if self.train_method=='blind':
                    ...
                    if idx_n.size(0) + idx_a.size(0) != score.size(0):
                        _, idx_a = torch.topk(score, round(score.shape[0] * (self.contamination+0.0001)), largest=True, sorted=False)
                        
#                     print("score:", score.shape)
#                     print("Truelables:",true_labels[true_labels==0].shape, true_labels[true_labels==1].shape)
...
#                    loss = torch.cat([loss_n[idx_n], loss_a[idx_a],loss_a[true_labels==1]], 0)
    ...
    def save_outliers(self, loader):
        ...
        return z_all , score_all, target_all,th
...
#                loss_in.item() / (target_all == 0).sum(), loss_out.item() / (target_all == 1).sum()
    ...
    def train(self, train_loader, contamination, query_num=0,optimizer=None, scheduler=None,
        ...
        for epoch in range(1, self.max_epochs+1):
           ...
           ####Save outliers#####         
#             if epoch ==1:
#                 z_all , score_all, target_all, th = self.save_outliers(test_loader)
#                 np.save('z_all_epoch1', z_all)
#                 np.save('score_all_epoch1', score_all)
#                 np.save('target_all_epoch1', target_all)
#                 np.save('th_epoch1', th)
                
#             elif epoch == 25:
#                 z_all , score_all, target_all, th = self.save_outliers(test_loader)
#                 np.save('z_all_epoch25', z_all)
#                 np.save('score_all_epoch25', score_all)
#                 np.save('target_all_epoch25', target_all)
#                 np.save('th_epoch25', th)
                
#             elif epoch == self.max_epochs:
#                 z_all , score_all, target_all, th = self.save_outliers(test_loader)
#                 np.save('z_all_epoch50', z_all)
#                 np.save('score_all_epoch50', score_all)
#                 np.save('target_all_epoch50', target_all)
#                 np.save('th_epoch50', th)


            if epoch == self.max_epochs:
                _ , score_all, _, _ = self.save_outliers(train_loader)
                np.save('score_all_train', score_all)
